...
function estimateBoxesHtml(est) {
  ...
  const hint = unavailable ? "<small class=\"estimate-hint\">缺少可用持仓或代理行情</small>" : official ? "<small class=\"estimate-hint\">官方已更新</small>" : "";
  const navLabel = "预估净值";
...
async function refreshEstimates() {
  ...
  try {
    ...
    const refreshHint = isMarketRefreshWindow() ? " · 开市中自动刷新" : "";
  ...
  els.watchRows.innerHTML = state.watchlist
    .map((item) => {
      ...
      if (!result.ok) {
        ...
        const actualDate = actualDateText(est);
        return `
        <tr data-code="${item.code}" class="${state.selectedCode === item.code ? "selected" : ""}">
          <td>
            <div class="fund-name">
...
function renderCard(item) {
  ...
  if (!result) {
    return `
      <article class="fund-card ${state.selectedCode === item.code ? "selected" : ""}" data-code="${item.code}">
...
async function addCode(code) {
  ...
  await api(`/api/watchlist/${normalized}`, { method: "POST" });
  els.searchResults.classList.remove("active");
...
setInterval(() => {
  if (isMarketRefreshWindow() && state.watchlist.length) {
