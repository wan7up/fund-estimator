...
PERSONAS: dict[str, tuple[str, str, str]] = {
    ...
    "dalio_balance": (
        "风险平衡风格",
        "偏资产暴露、相关性、组合分散和风险预算。",
        "参考风险平衡的分析方式，强调资产暴露、相关性、波动来源和组合角色；不要声称自己是具体投资人。",
    ),
    "marks_cycle": (
        "周期风险风格",
        "偏周期位置、下行风险和风险补偿。",
        "参考周期与风险控制的分析方式，强调潜在下行、拥挤度和风险补偿；不要声称自己是具体投资人。",
    ),
    "custom": (
        "自定义风格",
        "使用你填写的分析风格提示。",
        "使用用户提供的分析风格，但必须保持事实边界和投资风险提示。",
    ),
}
...
class CompareAiService:
    ...
    def _build_messages(self, request: CompareAiCommentaryRequest, config: CompareAiConfig) -> list[dict[str, str]]:
        ...
        user = (
            ...
            "取舍：只按已给评分/排序/推荐代码说明适合什么偏好，并点出关键风险。\n\n"
            "要求：如果 conclusion 是 not_comparable，不要给出谁更好的强推荐，取舍行只说明不能直接排序；"
